      ...
      <main className="flex-1 p-6 overflow-auto">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="font-display font-semibold text-gray-900 mb-2">All caught up!</h3>
...
function SubmissionCard({ item, provider }: { item: PortfolioItem; provider?: Provider }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden hover:shadow-md transition-shadow group">
      {/* Cover strip */}
      <div className="h-36 bg-gray-100 overflow-hidden relative">
        {item.cover_image ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={item.cover_image} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-display font-semibold text-gray-900 text-sm leading-snug line-clamp-2">{item.title}</h3>
          <span className="text-xs bg-amber-100 text-amber-700 font-semibold px-2.5 py-0.5 rounded-full flex-shrink-0 whitespace-nowrap">
            Pending
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span className="text-xs text-gray-400 capitalize">{templateLabel}</span>
        </div>

        {/* Provider info — internal admin view */}
        {provider && (
          <div className="flex items-center gap-1.5 mb-3 bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1.5">
            <svg className="w-3 h-3 text-amber-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
            </svg>
            <span className="text-xs text-amber-700 font-medium">{provider.name}</span>
            <span className="text-amber-300">·</span>
            <span className="text-xs text-amber-600">{provider.phone}</span>
          </div>
        )}
        ...
        <Link
          ...
          className="w-full inline-flex items-center justify-center gap-2 bg-[#0A0A0A] hover:bg-gray-800 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors"
        >
          Review →
        </Link>
      </div>
